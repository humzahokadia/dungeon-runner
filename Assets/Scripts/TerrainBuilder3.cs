...
public class TerrainBuilder3 : MonoBehaviour
{
    ...
    void SpawnNext(float angle)
    {
        ...
        if (cubes % 2 == 0)
        {
            newo = Instantiate(coin, new Vector3(nextPosX, nextPosY - 1, -2.4F), currentRotation);
            newo.gameObject.AddComponent<Destroy>();
        }
    ...
    public void SpawnEnemy()
    {
        ...
        for (int a = 0; a < powerUp.Length; a++)
        {
            if (a == b)
            {
                Quaternion currentRotation = new Quaternion();
                currentRotation.eulerAngles = new Vector3(0, 90, 0);
                newo = Instantiate(powerUp[a].transform, new Vector3(nextPosX + 5, nextPosY + 0.5F, -1.5F), currentRotation);
                newo.gameObject.AddComponent<Destroy>();
            }
        }
    }
