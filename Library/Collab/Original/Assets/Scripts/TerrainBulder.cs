...
public class TerrainBulder : MonoBehaviour
{
    ...
    public Transform[] enemies = new Transform[5];
    ...
    void SpawnNext(float angle)
    {
        ...
        if (cubes % 2 == 0)
        {
            newo = Instantiate(coin, new Vector3(nextPosX, nextPosY- 1, -2), currentRotation);
            newo.gameObject.AddComponent<Destroy>();
        }
    ...
    {   
        Transform newo;
        newo = Instantiate(enemySpawn, new Vector3(nextPosX + 8F, nextPosY, 0), Quaternion.identity);
        newo.gameObject.AddComponent<Destroy>();
        ...
        for (int a = 0; a < powerUp.Length; a++)
        {
            if (a == b)
            {
                Quaternion currentRotation = new Quaternion();
                currentRotation.eulerAngles = new Vector3(0, 90, 0);
                newo = Instantiate(powerUp[a].transform, new Vector3(nextPosX + 5, nextPosY + 3F, -2.4f), currentRotation);
                newo.gameObject.AddComponent<Destroy>();
            }
        }
